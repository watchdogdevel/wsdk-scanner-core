...
cl_error_t cl_scanhandle(HANDLE handle, const char *filename, const char **virname, unsigned long int *scanned, const struct cl_engine *engine, struct cl_scan_options *scanoptions)
{
	return cl_scanhandle_callback(handle, filename, virname, scanned, engine, scanoptions, NULL);
}

cl_error_t cl_scanhandle_callback(HANDLE handle, const char *filename, const char **virname, unsigned long int *scanned, const struct cl_engine *engine, struct cl_scan_options *scanoptions, void *context)
{
	int fd;
	cl_error_t ret;
	...
	if ((fd = _open_osfhandle((intptr_t)handle, O_RDONLY | O_BINARY)) == -1)
		return CL_EOPEN;

	ret = cl_scandesc_callback(fd, filename, virname, scanned, engine, scanoptions, NULL);
	close(fd);
